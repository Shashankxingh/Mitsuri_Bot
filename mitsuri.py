...
# === Owner and group IDs ===
OWNER_ID = 8162412883
SPECIAL_GROUP_ID = -1002759296936  # Your special group ID
...
def generate_with_retry(messages, retries=2, delay=REQUEST_DELAY):
...
def ping(update: Update, context: CallbackContext):
    ...
    try:
        ...
        deepinfra_reply = response.choices[0].message.content.strip()
...
def show_callback(update: Update, context: CallbackContext):
    ...
    if data == "back_to_menu":
        query.edit_message_text("Choose chat type:", reply_markup=get_main_menu_buttons())
        return
    ...
    if data.startswith("forget_"):
        ...
        # Enhanced forget functionality with group leaving and user blocking
